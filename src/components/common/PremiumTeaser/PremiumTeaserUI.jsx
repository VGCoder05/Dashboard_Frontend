...
const PremiumTeaserUI = ({ 
  ...
  return (
    <div className="m-3 relative overflow-hidden bg-amber-400 rounded-xl p-6 text-white mb-6 shadow-custom-light">
      {/* Background pattern */}
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mb-4 text-white/90 font-display">{description}</p>
            <ul className="space-y-1 mb-4">
              {highlights.map((item, i) => (
                <li
                  key={i}
                  className="flex items-center text-sm text-white/80 font-display"
                >
                  <svg
